...
export default function VideoModule({ videos, s }: VideoModuleProps) {
  ...
  return (
    <div>
      ...
      {allVideos.length === 0 ? (
        <p className="text-[#999] text-[14px] py-8" style={{ fontFamily: "var(--font-serif)" }}>暂无课程视频，请上传</p>
      ) : (
        <div>
          ...
          <div className="flex items-center gap-2 mb-5">
            <span className="text-[12px] text-[#999]" style={{ fontFamily: "var(--font-serif)" }}>{s.playback_speed}:</span>
            {SPEEDS.map((sp) => (
              <button key={sp} onClick={() => handleSpeedChange(sp)}
                className={`px-2.5 py-1 rounded-[6px] text-[12px] border cursor-pointer transition-colors ${speed === sp ? "bg-[#333] text-white border-[#333]" : "bg-white text-[#666] border-[#ccc] hover:border-[#666]"}`}
            ...
            {allVideos.map((v, i) => {
              ...
              return (
                <div key={v.id}
                  ...
                  <span className="text-[11px] text-[#999] uppercase" style={{ fontFamily: "var(--font-display)" }}>{v.type}</span>
                  {isUser && (
