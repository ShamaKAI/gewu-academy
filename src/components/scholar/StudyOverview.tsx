...
export default function StudyOverview() {
  ...
  const recentCourses = [
    { name: "《大学》精读", timeKey: "recent_time_1", progress: 78, cover: "https://picsum.photos/seed/great-learning-cover/400/250" },
    { name: "风险管理基础", timeKey: "recent_time_2", progress: 60, cover: "https://picsum.photos/seed/risk-cover/400/250" },
    { name: "数据分析导论", timeKey: "recent_time_3", progress: 45, cover: "https://picsum.photos/seed/data-sci-cover/400/250" },
    { name: "金融数学建模", timeKey: "recent_time_4", progress: 92, cover: "https://picsum.photos/seed/finance-cover/400/250" },
  ];
    ...
    <div className="px-5 py-6">
      ...
      <div className="flex flex-col gap-3 mb-6">
        {overviewItems.map((item) => (
          <div
            key={item.label}
            className="flex items-center gap-3 bg-white rounded-[12px] p-3.5 border border-[#000]"
          >
      ...
      <div className="flex flex-col gap-2">
        {recentCourses.map((rc, i) => (
          <div
            key={i}
            className="flex items-center gap-3 bg-white rounded-[10px] p-3 border border-[#000] cursor-pointer hover:border-[#000] hover:shadow-sm transition-all duration-200"
          >
            <img
              src={rc.cover}
              alt={rc.name}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <div className="flex-1 h-[5px] bg-[#000] rounded-full">
                  <div className="h-full bg-[#000] rounded-full" style={{ width: `${rc.progress}%` }} />
                </div>
                <span className="text-[10px] text-[#000]" style={{ fontFamily: "var(--font-display)" }}>{rc.progress}%</span>
              </div>
            </div>
