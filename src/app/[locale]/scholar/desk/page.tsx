...
export default function DeskPage() {
  ...
  const setAnswer = (exId: string, val: string | string[]) => {
    if (submitted.has(exId)) return;
    setAnswers((prev) => ({ ...prev, [exId]: val }));
  };

  const submitAnswer = (exId: string) => {
    setSubmitted((prev) => { const n = new Set(prev); n.add(exId); return n; });
  };

  const isCorrect = (ex: Exercise & { courseTitle: string; chapterTitle: string }): boolean | null => {
    if (!submitted.has(ex.id)) return null;
  ...
  const exercisesContent = (
    <div className="space-y-5 max-w-2xl">
      {allExercises.map((ex, i) => {
        ...
        return (
          <motion.div key={`${ex.id}-${i}`} className={`p-5 rounded-[12px] border ${correct===true?"bg-[#f0faf0] border-[#388e3c]":correct===false?"bg-[#fef5f5] border-[#C04040]":"bg-white border-[#000]"}`}
            ...
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1">
                <p className="text-[15px] text-[#000] font-bold m-0" style={{ fontFamily: "var(--font-serif)" }}>{ex.question}</p>
                <p className="text-[11px] text-[#000] m-0 mt-1 opacity-50" style={{ fontFamily: "var(--font-serif)" }}>
            ...
            {ex.type === "fill" ? (
              <div className="ml-7 flex gap-2">
                <input type="text" disabled={submitted.has(ex.id)} value={(answers[ex.id] as string) || ""}
                  onChange={(e) => setAnswer(ex.id, e.target.value)}
                  placeholder="输入答案..."
                  className="flex-1 px-4 py-2.5 rounded-[8px] border border-[#000] text-[14px] text-[#000] outline-none disabled:opacity-50"
                  style={{ fontFamily: "var(--font-serif)" }} />
                <button onClick={() => submitAnswer(ex.id)} disabled={submitted.has(ex.id) || !answers[ex.id]}
                  className="px-4 py-2.5 rounded-[8px] text-[13px] font-bold border-none cursor-pointer bg-[#000] text-white hover:bg-[#333] disabled:opacity-30 disabled:cursor-not-allowed"
                  style={{ fontFamily: "var(--font-serif)" }}>提交</button>
              </div>
            ) : (
              <div className="ml-7 space-y-1.5">
                {ex.options?.map((opt) => {
                  const selected = ex.type === "multi" ? ((answers[ex.id] as string[]) || []).includes(opt) : answers[ex.id] === opt;
                  return (
                    <button key={opt} disabled={submitted.has(ex.id)}
                      onClick={() => {
                        if (ex.type === "multi") {
                          const cur = (answers[ex.id] as string[]) || [];
                          setAnswer(ex.id, cur.includes(opt) ? cur.filter((o) => o !== opt) : [...cur, opt]);
                        } else {
                          setAnswer(ex.id, opt);
                        }
                      }}
                      className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-[8px] text-[14px] text-left border cursor-pointer transition-colors ${selected?"bg-[#e8e8e8] border-[#000] font-bold":"bg-white border-[#ccc] text-[#000] hover:border-[#000]"} disabled:cursor-default`}
                      style={{ fontFamily: "var(--font-serif)" }}>
                      <span className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${selected?"border-[#000]":"border-[#ccc]"}`}>
                        {selected && <span className="w-2 h-2 rounded-full bg-[#000]" />}
                      </span>
                      {opt}
                    </button>
                  );
                ...
                <button onClick={() => submitAnswer(ex.id)} disabled={submitted.has(ex.id) || !answers[ex.id]}
                  className="mt-2 px-4 py-2 rounded-[8px] text-[13px] font-bold border-none cursor-pointer bg-[#000] text-white hover:bg-[#333] disabled:opacity-30 disabled:cursor-not-allowed"
                  style={{ fontFamily: "var(--font-serif)" }}>提交</button>
              </div>
            )}
            {correct !== null && (
              <p className={`text-[13px] m-0 mt-3 ml-7 font-bold ${correct?"text-[#388e3c]":"text-[#C04040]"}`} style={{ fontFamily: "var(--font-serif)" }}>
                {correct ? "✓ 正确" : `✗ 错误 · 正确答案：${Array.isArray(ex.answer)?ex.answer.join(", "):ex.answer}`}
              </p>
            )}
          </motion.div>
        );
        ...
        mockMistakes.map((m, i) => (
          <motion.div key={m.id} className="p-5 bg-white rounded-[12px] border border-[#000] border-l-[4px] border-l-[#C04040]"
            ...
            <div className="flex items-start gap-3 mb-2">
              <span className="text-[13px] font-bold flex-shrink-0 mt-[3px]" style={{ fontFamily: "'Times New Roman', serif" }}>{i + 1}.</span>
              <div className="flex-1">
                <p className="text-[15px] text-[#000] font-bold m-0" style={{ fontFamily: "var(--font-serif)" }}>{m.exercise.question}</p>
              </div>
            </div>
            <div className="ml-8 flex flex-col gap-1">
              ...
              <p className="text-[10px] text-[#000] m-0 opacity-50" style={{ fontFamily: "var(--font-serif)" }}>{m.courseTitle} · {m.date}</p>
            </div>
          </motion.div>
        ))
      )}
      ...
      {pendingTodos.length > 0 && (
        <div>
          ...
          <div className="space-y-2">
            {pendingTodos.map((td) => (
              <motion.div key={td.id} className="flex items-center gap-3 p-4 bg-white rounded-[10px] border border-[#000] cursor-pointer"
