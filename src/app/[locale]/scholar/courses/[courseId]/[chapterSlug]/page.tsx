  ...
  if (!course || !chapter) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-[#000] text-[16px]" style={{ fontFamily: "var(--font-serif)" }}>章节不存在</p>
      </div>
    );
  }
    ...
    <motion.div className="flex h-full" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.4, 0, 0.2, 1] }}>
      ...
      <div className="w-[200px] flex-shrink-0 flex flex-col border-r border-[#eee] bg-[#fafafa]">
        <button onClick={() => router.push(`/${locale}/scholar/courses/${courseId}`)}
          className="flex items-center gap-2 px-4 py-4 text-[13px] text-[#000] hover:text-[#333] bg-transparent border-b border-[#eee] cursor-pointer transition-colors"
          style={{ fontFamily: "var(--font-serif)" }}>← {course.title}</button>

        <div className="px-4 py-3 border-b border-[#eee]">
          <p className="text-[11px] text-[#000] m-0 mb-0.5" style={{ fontFamily: "var(--font-serif)" }}>{chapter.title}</p>
          <div className="flex gap-2 mt-2">
            {prevChapter && <button onClick={() => router.push(`/${locale}/scholar/courses/${courseId}/${prevChapter.slug}`)} className="text-[11px] text-[#000] hover:text-[#333] bg-transparent border-none cursor-pointer px-0" style={{ fontFamily: "var(--font-serif)" }}>← 上一章</button>}
            {nextChapter && <button onClick={() => router.push(`/${locale}/scholar/courses/${courseId}/${nextChapter.slug}`)} className="text-[11px] text-[#000] hover:text-[#333] bg-transparent border-none cursor-pointer ml-auto px-0" style={{ fontFamily: "var(--font-serif)" }}>下一章 →</button>}
          </div>
        </div>
        ...
        <nav className="flex-1 flex flex-col py-2">
          {TAB_ICONS.map((key) => (
            <button key={key} onClick={() => setActiveTab(key)}
              className={`flex items-center gap-3 px-5 py-3 text-[14px] text-left border-none cursor-pointer transition-all duration-200 bg-transparent ${activeTab === key ? "text-[#000] font-bold bg-white border-r-[3px] border-[#333]" : "text-[#000] hover:text-[#333] hover:bg-white/50 border-r-[3px] border-transparent"}`}
